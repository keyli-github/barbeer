...
class _VentaDetailSheetState extends ConsumerState<VentaDetailSheet> {
  ...
  Future<void> _anular() async {
    ...
    setState(() { _anulando = true; _errorAnular = null; });
    try {
      await ref.read(ventasRepositoryProvider).anularVenta(widget.venta.id, motivo: 'Anulada manualmente');
      if (mounted) {
      ...
      setState(() { _anulando = false; _errorAnular = e.toString(); });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = venta.isAnulada ? AppColors.error
                ...
                children: [
                  ...
                  DSCard(
                    child: Column(children: [
                      ...
                      const Divider(height: 16),
                      _InfoRow('Fecha', dt != null ? FormatUtils.dateTime(dt) : '—'),
                      const Divider(height: 16),
                      _InfoRow('Total', FormatUtils.currency(venta.total),
                    ...
                    DSCard(
                      child: Column(children: [
                        ...
                        if (venta.conciliacion!.etiquetaNombre != null) ...[
                          const Divider(height: 16),
                          _InfoRow('Billetera', venta.conciliacion!.etiquetaNombre!),
                        ],
                        if (venta.conciliacion!.comprobante != null) ...[
                          const Divider(height: 16),
                          _InfoRow('Comprobante', venta.conciliacion!.comprobante!),
                        ],
                      ]),
                    ),
                  ],
                          ...
                          Padding(
                            ...
                            child: Row(children: [
                              ...
                              Expanded(child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(item.productoNombre ?? item.productoCodigo ?? '—',
                                    style: const TextStyle(fontSize: 14,
                                      fontWeight: FontWeight.w600,
                                      color: AppColors.textPrimary),
                                    maxLines: 2, overflow: TextOverflow.ellipsis),
                                  Text('${item.cantidad} × ${FormatUtils.currency(item.precioUnitario)}',
                  ...
                  DSCard(
                    child: Column(children: [
                      ...
                      const Divider(height: 16),
                      _InfoRow('Sesión de caja', venta.cajaSesionId, monospace: true),
                    ]),
                  ),
...
class _InfoRow extends StatelessWidget {
  ...
  const _InfoRow(this.label, this.value, {this.monospace = false, this.valueStyle});
  @override
