                ...
                children: [
                  ...
                  RichText(
                    text: const TextSpan(children: [
                      TextSpan(text: 'Bar', style: TextStyle(
                        fontSize: 20, fontWeight: FontWeight.w800,
                        color: AppColors.textPrimary, height: 1.1)),
                      TextSpan(text: 'Beer', style: TextStyle(
                        fontSize: 20, fontWeight: FontWeight.w800,
                        color: AppColors.brand, height: 1.1)),
                    ]),
                  ),
                  Row(
                    children: [
...
class _AdminContent extends StatelessWidget {
  ...
  const _AdminContent({required this.rol, required this.ref, required this.auth});

  @override
  Widget build(BuildContext context) {
    final dash = ref.watch(dashboardProvider);

    if (dash.isLoading) return const SliverToBoxAdapter(child: DSSkeletonList(count: 6));
    ...
    return SliverList(
      delegate: SliverChildListDelegate([
        ...
        // ── Sedes recientes ───────────────────────────────────────────────
        if (sedes.isNotEmpty) ...[
          _SectionTitle('Sedes', action: 'Ver todas',
            onAction: auth.hasPermission('establecimientos:leer')
              ? () => GoRouter.of(context).go('/sucursales') : null),
        ...
        // ── Actividad reciente ────────────────────────────────────────────
        if (audit.isNotEmpty) ...[
          _SectionTitle('Actividad reciente', action: 'Ver todo',
            onAction: auth.hasPermission('audit:leer')
              ? () => GoRouter.of(context).go('/auditoria') : null),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: AppSpacing.md),
            child: DSCard(
              ...
              child: Column(
                children: audit.take(5).toList().asMap().entries.map((e) {
                  final i = e.key; final log = e.value as Map;
                  return Column(
                    children: [
                      _AuditRow(log: log),
        ...
        if (actual != null && actual.resumen != null) ...[
          Padding(
            ...
            child: Row(children: [
              Expanded(child: DSStatCard(
                label: 'Ventas neto',
                value: FormatUtils.currency(actual.resumen!.v2?.totalVentasNeto ?? 0),
          ...
          Padding(
            ...
            child: Row(children: [
              Expanded(child: DSStatCard(
                ...
                value: FormatUtils.currency(actual.resumen!.efectivoEsperado),
              ...
              Expanded(child: DSStatCard(
                label: 'Digital neto',
                value: FormatUtils.currency(actual.resumen!.v2?.totalDigitalNeto ?? 0),
      ...
      delegate: SliverChildListDelegate([
        ...
        Padding(
          ...
          child: DSCard(
            child: Column(
              children: [
                Icon(Icons.storefront_rounded, size: 48,
                  color: AppColors.primary.withValues(alpha: 0.5)),
                const SizedBox(height: AppSpacing.sm),
                const Text('Registra ventas fácilmente',
                  style: TextStyle(
      ...
      child: DSCard(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(sede['nombre'] as String? ?? '—',
                style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600,
                  color: AppColors.textPrimary)),
              Text(sede['codigo'] as String? ?? '',
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: (activo ? AppColors.success : AppColors.textTertiary).withValues(alpha: 0.12),
              borderRadius: BorderRadius.circular(20),
            ),
            child: Text(activo ? 'Activa' : 'Inactiva',
              style: TextStyle(
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(accion, style: const TextStyle(
              fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textPrimary),
              maxLines: 1, overflow: TextOverflow.ellipsis),
            Text(usuario, style: const TextStyle(fontSize: 12, color: AppColors.textTertiary)),
          ],
        )),
        if (dt != null)
          Text(FormatUtils.timeAgo(dt),
    ...
    return DSCard(
      child: Row(children: [
        Container(
          width: 48, height: 48,
          decoration: BoxDecoration(
            color: (abierta ? AppColors.success : AppColors.textTertiary).withValues(alpha: 0.12),
            borderRadius: BorderRadius.circular(14),
          ),
          child: Icon(
            abierta ? Icons.account_balance_wallet_rounded : Icons.wallet_rounded,
          ...
          children: [
            Text(abierta ? 'Caja abierta' : 'Caja cerrada',
              style: TextStyle(
                ...
                color: abierta ? AppColors.success : AppColors.textSecondary)),
            if (abierta && actual.apertura != null)
              Text('Apertura: ${FormatUtils.currency(actual.apertura!)}',
                style: const TextStyle(fontSize: 12, color: AppColors.textTertiary)),
          ],
        )),
...
class _QuickActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (auth.hasPermission('ventas:leer') || auth.hasPermission('ventas:leer-propias'))
      actions.add(_QA('Ventas', Icons.shopping_cart_rounded, AppColors.primary, '/ventas'));
    if (auth.hasPermission('caja:leer'))
      actions.add(_QA('Caja', Icons.account_balance_wallet_rounded, AppColors.success, '/caja'));
    if (auth.hasPermission('productos:crear'))
      actions.add(_QA('Productos', Icons.liquor_rounded, AppColors.brand, '/productos'));
    if (auth.hasPermission('inventario:leer'))
      actions.add(_QA('Inventario', Icons.inventory_2_rounded, AppColors.info, '/inventario'));
    if (auth.hasPermission('usuarios:leer'))
      actions.add(_QA('Usuarios', Icons.people_rounded, AppColors.warning, '/usuarios'));

    if (actions.isEmpty) return const SizedBox.shrink();

    return Row(
      children: actions.take(4).map((a) => Expanded(
        child: Padding(
          padding: EdgeInsets.only(right: actions.indexOf(a) < actions.take(4).length - 1 ? 8 : 0),
          child: GestureDetector(
            onTap: () { HapticFeedback.selectionClick(); GoRouter.of(context).go(a.path); },
            child: Container(
              height: 72,
              decoration: BoxDecoration(
