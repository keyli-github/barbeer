...
/// Opens the PIN management bottom sheet for a list of user maps.
/// Each map must contain: id, username, rolNombre (or rol.nombre), nivelAcceso.
Future<void> showPinManagementSheet(
  ...
  return showModalBottomSheet<void>(
    ...
    builder: (_) => _PinSheet(users: users),
  );
}

class _PinSheet extends ConsumerStatefulWidget {
  final List<Map<String, dynamic>> users;
  const _PinSheet({required this.users});
  @override
  ConsumerState<_PinSheet> createState() => _PinSheetState();
}

class _PinSheetState extends ConsumerState<_PinSheet> {
  late List<_PinEntry> _entries;
  String _search = '';
  final _repo = UsuarioAdminRepository(ApiClient.instance);

  @override
  void initState() {
    super.initState();
    _entries = widget.users.map((u) {
      final rol = (u['rol'] is Map ? u['rol']['nombre'] : u['rol']) as String? ?? '';
      return _PinEntry(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(children: [
              const Expanded(
                  child: Text('Gestionar claves PIN',
                      style: TextStyle(
          ...
          ConstrainedBox(
            ...
            child: filtered.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (_, i) =>
                        _PinRow(entry: filtered[i], onSave: _save,
                          onChanged: (updated) => setState(() {
                            final idx = _entries.indexWhere((e) => e.userId == updated.userId);
...
class _PinRow extends StatefulWidget {
  ...
  const _PinRow({required this.entry, required this.onSave, required this.onChanged});
  @override
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(e.username, style: const TextStyle(fontWeight: FontWeight.bold)),
              Text(e.rol, style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
            ]),
          ),
          // Mode toggle
        ...
        if (e.autoGenerate) ...[
          ...
          if (e.pin.isNotEmpty)
            ...
            Text('Clave dinámica que rota cada 24 horas.',
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
                ...
                border: OutlineInputBorder()),
            onChanged: (v) =>
                widget.onChanged(e.copyWith(pin: v)),
          ),
          const SizedBox(height: 4),
          ElevatedButton.icon(
