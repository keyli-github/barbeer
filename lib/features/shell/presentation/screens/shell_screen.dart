...
const _nav = [
  ...
  // Stock se agrega de forma condicional para no exponer inventario a
  // VENDEDORA ni CAJERO, cuyos permisos no incluyen inventario:leer.
  _NavItem(
    path: '/perfil',
    label: 'Perfil',
...
const _drawer = [
  _DSec(
    title: 'PRINCIPAL',
    items: [
      _DItem(path: '/dashboard', label: 'Dashboard', icon: Icons.home_rounded),
    ],
  ),
  _DSec(
    title: 'VENTAS Y CAJA',
    items: [
      _DItem(path: '/ventas', label: 'Ventas', icon: Icons.storefront_rounded),
      _DItem(
        path: '/caja',
        label: 'Caja',
  ...
  _DSec(
    ...
    items: [
      _DItem(
        ...
        // La consulta de productos para VENDEDORA ocurre dentro de Ventas;
        // esta entrada es la gestión administrativa del catálogo.
...
class _NavBar extends StatelessWidget {
  ...
  const _NavBar({required this.current, required this.go, required this.items});
  @override
  Widget build(BuildContext context) {
    final bot = MediaQuery.of(context).padding.bottom;
    return Padding(
      padding: EdgeInsets.only(left: 12, right: 12, bottom: bot + 10),
      child: Container(
        height: 68,
...
List<_NavItem> _navFor(AuthState auth) {
  final items = <_NavItem>[..._nav];
            ...
            Padding(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    radius: 26,
                    backgroundColor: AppColors.avatarColor(un),
                    child: Text(
                  ...
                  for (final sec in _drawer) ...[
                    ...
                    for (final item in sec.items)
                      if (item.perm == null || auth.hasPermission(item.perm!))
                        _DNav(
                          item: item,
                          on: current.startsWith(item.path),
                          tap: () {
