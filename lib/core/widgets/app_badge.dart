...
class PermissionModuleBadge extends StatelessWidget {
  final String module;
  const PermissionModuleBadge({super.key, required this.module});
  static const _c = {
    'usuarios': Color(0xFF2563EB),
    'roles': Color(0xFF8B5CF6),
    'permisos': Color(0xFFF59E0B),
    'audit': Color(0xFF10B981),
    'establecimientos': Color(0xFFF97316),
  };
  @override
  Widget build(BuildContext context) {
...
class AuditActionBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Color c;
    if (action.contains('LOGIN') || action.contains('SESION'))
      c = const Color(0xFF2563EB);
    else if (action.contains('CREAR'))
      c = const Color(0xFF10B981);
    else if (action.contains('EDITAR') ||
        action.contains('CAMBIAR') ||
        action.contains('ASIGNAR') ||
        action.contains('RESETEAR'))
