        ...
        home: Builder(
          builder: (ctx) => Scaffold(
            body: ElevatedButton(
              ...
              onPressed: () => showPinManagementSheet(ctx, repo: repo),
...
void main() {
  ...
  group('Scenario 36 — PIN denial does not expose the secret', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final pinInPlainText = find.byWidgetPredicate(
          (w) => w is Text && (w.data?.contains(enteredPin) ?? false),
        );
        expect(
          pinInPlainText,
          findsNothing,
  ...
  group('Scenario 35 — PinManagementSheet coverage', () {
    testWidgets('happy path: sheet renders title and user list with roles', (
      ...
      expect(
        find.text('Gestionar claves PIN'),
        findsOneWidget,
