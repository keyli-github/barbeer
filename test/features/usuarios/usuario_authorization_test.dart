...
Map<String, dynamic> _resp({
  ...
  final catalog = [
    ...
    },
    {
      ...
      'nombre': 'cuentas:leer',
      'modulo': 'cuentas',
      'descripcion': 'Leer cuentas',
    },
    {
      'id': 'p-inv-leer',
      'nombre': 'inventario:leer',
      'modulo': 'inventario',
      'descripcion': 'Leer inventario',
    },
  ];
  return {
    'usuario': {'id': 'u-1', 'username': 'maria', 'rol': 'VENDEDORA'},
    'permisosPorRol': catalog.where((p) => roleIds.contains(p['id'])).toList(),
    'permisosAdicionales': catalog
        .where((p) => addSet.contains(p['id']))
        .toList(),
    'permisosRevocados': catalog
        .where((p) => revSet.contains(p['id']))
        .toList(),
...
void main() {
  group('permission', () {
    test(
      'DTO maps GET response with inherited, granted, revoked, and effective',
      () {
        ...
        final inherited = r.permisosEfectivos.firstWhere(
          (e) => e.id == 'p-ventas-leer',
        );
        expect(
          (inherited.porRol, inherited.adicional, inherited.activo),
          (true, false, true),
        );
        final granted = r.permisosEfectivos.firstWhere(
          (e) => e.id == 'p-cuentas-leer',
        );
        expect(
          (granted.porRol, granted.adicional, granted.activo),
          (false, true, true),
        );
        final revoked = r.permisosEfectivos.firstWhere(
          (e) => e.id == 'p-ventas-crear',
        );
        expect(
          (revoked.porRol, revoked.revocado, revoked.activo),
          (true, true, false),
        );
      },
    );
    ...
    test(
      'replace payload serializes exact PUT body and omits empty revocations',
      () {
        final full = ReplacePermissionsPayload(
          permisoIds: ['p-cuentas-leer', 'p-inv-leer'],
          permisoIdsRevocados: ['p-ventas-crear'],
        );
        ...
        final noRevocations = ReplacePermissionsPayload(
          permisoIds: ['p-cuentas-leer'],
        );
        expect(
          noRevocations.toJson().containsKey('permisoIdsRevocados'),
          isFalse,
        );
      },
    );
    ...
    test('catalog permission parses id, nombre, modulo, and descripcion', () {
      ...
      expect(
        (p.id, p.nombre, p.modulo, p.descripcion),
        ('p-1', 'ventas:leer', 'ventas', 'Leer ventas'),
      );
    });

    test(
      'effective semantics: activo = (porRol || adicional) && !revocado',
      () {
    ...
    test(
      'repository GET returns parsed response from exact endpoint',
      () async {
        final paths = <String>[];
        final repo = UsuarioAdminRepository(
          ApiClient.instance,
    ...
    test('repository PUT sends payload and returns updated model', () async {
      ...
      final repo = UsuarioAdminRepository(
        ApiClient.instance,
        putRequest: (p, b) async {
          path = p;
          body = b;
          return _resp(added: ['p-cuentas-leer']);
        },
      );
      final result = await repo.replacePermissions(
        'u-1',
    ...
    test('repository surfaces 403 and 400 backend errors', () async {
      final denied = UsuarioAdminRepository(
        ApiClient.instance,
        putRequest: (_, __) async {
          throw const AppException(
            message: 'Solo el SUPERADMIN puede modificar permisos.',
            statusCode: 403,
          );
        },
      );
      expect(
        () => denied.replacePermissions(
          'u-1',
          ReplacePermissionsPayload(permisoIds: ['p-1']),
        ),
        throwsA(isA<AppException>().having((e) => e.statusCode, 'status', 403)),
      );
      final invalid = UsuarioAdminRepository(
        ApiClient.instance,
        putRequest: (_, __) async {
          throw const AppException(
            message: 'Uno o mas permisos no existen en el catalogo.',
            statusCode: 400,
          );
        },
      );
      expect(
        () => invalid.replacePermissions(
          'u-1',
          ReplacePermissionsPayload(permisoIds: ['bad']),
        ),
    ...
    test(
      'permission harness: create, replace, and delete exception flows',
      () async {
        ...
        final repo = UsuarioAdminRepository(
          ...
          putRequest: (path, body) async {
            ...
            final revs =
                (body['permisoIdsRevocados'] as List?)?.cast<String>() ?? [];
        ...
        expect(
          r.permisosEfectivos.where((e) => e.activo).map((e) => e.id).toSet(),
          {'p-ventas-leer', 'p-ventas-crear'},
        );
        // Create: grant cuentas:leer
        r = await repo.replacePermissions(
          'u-1',
          ReplacePermissionsPayload(permisoIds: ['p-cuentas-leer']),
        );
        ...
        expect(
          r.permisosEfectivos
              .firstWhere((e) => e.id == 'p-ventas-crear')
              .activo,
          isFalse,
        );
        ...
        r = await repo.replacePermissions(
          'u-1',
          ReplacePermissionsPayload(permisoIds: []),
        );
        expect(r.permisosAdicionales, isEmpty);
  ...
  group('PIN', () {
    ...
    test(
      'StockAdjustPayload serializes all fields including superadminPin',
      () {
    ...
    test(
      'StockAdjustResult parses productoId, sedeId, stock, tipo, cantidad',
      () {
        ...
        expect(
          (r.productoId, r.sedeId, r.stock, r.tipo, r.cantidad),
          ('prod-1', 's-1', 18, 'ENTRADA', 10),
        );
      },
    );

    test('API constants build superadmin PIN, validation, and stock paths', () {
      expect(ApiConstants.superadminPin('u-x'), '/usuarios/u-x/superadmin-pin');
      expect(ApiConstants.superadminPins, '/usuarios/superadmin-pins');
      expect(ApiConstants.validatePin, '/usuarios/validate-pin');
      expect(ApiConstants.productStock('p-y'), '/productos/p-y/stock');
    });

    test(
      'repository PATCH configures superadmin PIN at exact endpoint',
      () async {
        String? path;
        Map<String, dynamic>? body;
        final repo = UsuarioAdminRepository(
          ApiClient.instance,
          patchRequest: (p, b) async {
            path = p;
            body = b;
            return {'message': 'ok'};
          },
        );
        await repo.configureSuperadminPin(
          'u-1',
    ...
    test('repository GET loads server-managed PIN configurations', () async {
      String? path;
      final repo = UsuarioAdminRepository(
        ApiClient.instance,
        getRequest: (value) async {
          path = value;
          return [
            {
              'id': 'u-1',
              'username': 'admin',
              'pinAutoGenerate': true,
              'currentPin': '1234',
            },
          ];
        },
      ...
      final pins = await repo.getSuperadminPins();

      expect(path, '/usuarios/superadmin-pins');
      expect(pins.single['currentPin'], '1234');
    });
    ...
    test('repository surfaces 429 throttle on excessive PIN attempts', () async {
      ...
      expect(
        () => repo.validatePin('0000'),
        throwsA(isA<AppException>().having((e) => e.statusCode, 'status', 429)),
      );
    });

    test(
      'repository POST adjustStock sends payload with PIN authorization',
      () async {
        String? path;
        Map<String, dynamic>? body;
        final repo = UsuarioAdminRepository(
    ...
    test(
      'repository surfaces 403 on stock without PIN and 400 on negative stock',
      () async {
        final noPinRepo = UsuarioAdminRepository(
          ApiClient.instance,
          postRequest: (_, __) async {
            throw const AppException(
              message:
                  'Se requiere el PIN de un superadministrador para ajustar stock',
              statusCode: 403,
            );
          },
        );
        expect(
          () => noPinRepo.adjustStock(
            'p-1',
            StockAdjustPayload(tipo: 'SALIDA', cantidad: 5),
          ),
          throwsA(
            isA<AppException>().having((e) => e.statusCode, 'status', 403),
          ),
        );
        final negativeRepo = UsuarioAdminRepository(
          ApiClient.instance,
          postRequest: (_, __) async {
            throw const AppException(
        ...
        expect(
          ...
          throwsA(
            isA<AppException>().having((e) => e.statusCode, 'status', 400),
          ),
        );
      },
    );
    ...
    test(
      'PIN harness: throttled PIN entry, then stock-check authorization',
      () async {
        ...
        final s = await repo.adjustStock(
          'prod-1',
          StockAdjustPayload(
            tipo: 'SALIDA',
            cantidad: 2,
        ...
        expect(
          () => repo.validatePin('0000'), // attempt 5 → throttle
          throwsA(
            isA<AppException>().having((e) => e.statusCode, 'status', 429),
          ),
        );
  ...
  Widget _editorApp(UsuarioAdminRepository repo) => MaterialApp(
    home: Scaffold(
      body: PermissionEditorSheet(userId: 'u-1', username: 'maria', repo: repo),
    ),
  );

  group('permission editor widget', () {
    testWidgets(
      'loads permissions grouped by module with inherited/granted/revoked',
      (tester) async {
        var current = _resp(
          added: ['p-cuentas-leer'],
          revoked: ['p-ventas-crear'],
        );
        final repo = UsuarioAdminRepository(
          ApiClient.instance,
          getRequest: (_) async => current,
          putRequest: (_, b) async {
            current = _resp(
              added: (b['permisoIds'] as List).cast<String>(),
              revoked:
                  (b['permisoIdsRevocados'] as List?)?.cast<String>() ?? [],
            );
            return current;
          },
        );
    ...
    testWidgets('toggle calls replacePermissions and updates state', (
      tester,
    ) async {
    ...
    testWidgets('module bulk toggle enables all permissions in a module', (
      tester,
    ) async {
      Map<String, dynamic>? lastPutBody;
      var current = _resp();
    ...
    testWidgets('shows 403 error from non-superadmin', (tester) async {
      final repo = UsuarioAdminRepository(
        ...
        getRequest: (_) async {
          throw const AppException(
            message: 'Solo el SUPERADMIN puede ver permisos.',
            statusCode: 403,
          );
        },
      );
  ...
  group('PIN stock widget', () {
    testWidgets('valid PIN then successful stock adjustment shows result', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        _pinStockApp(repo, onSaved: () => savedCalled = true),
      );
      // Select ENTRADA
    ...
    testWidgets('wrong PIN shows error and keeps form editable', (
      tester,
    ) async {
      final repo = UsuarioAdminRepository(
        ApiClient.instance,
