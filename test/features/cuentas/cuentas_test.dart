      ...
      'pendientes': [
        {
          ...
          'montoPendiente': balance,
          'totalVenta': 25,
          'recargoMonto': 5,
          'recargoMotivo': 'private',
          'sede': {'id': 's1', 'nombre': 'Centro'},
          'items': [
            {
              'id': 'i1',
              'cantidad': 2,
...
Map<String, dynamic> paidDetail(String id, double balance) => {
      ...
      'movimientos': [
        {
          ...((detail(id, 'Ana', balance)['movimientos'] as List).single
              as Map),
          'tipo': 'ABONO',
          'monto': 4,
          'medioPago': 'EFECTIVO'
...
void main() {
  test('read repository maps exact list/detail DTOs and query names', () async {
    ...
    final repo =
        CuentasRepository(ApiClient.instance, request: (path, query) async {
    ...
    expect((
      selected.movimientos.single.tipo,
      selected.movimientos.single.monto,
      selected.movimientos.single.referencia
    ), (
      'CARGO',
      12.5,
      'V-01'
    ...
    expect((
      selected.pendientes.single.codigo,
      selected.pendientes.single.totalVenta,
      selected.pendientes.single.items.single.producto.nombre
    ), (
      'V-01',
      25,
      'Producto'
  ...
  test('read state distinguishes empty, denied, content, and missing detail',
      () async {
    var calls = 0;
    final denied = CuentasNotifier(
        CuentasRepository(ApiClient.instance, request: (_, __) async {
          calls++;
          return [];
        }),
        authorized: false,
    ...
    expect((
      calls,
      (denied.state.listState as OperationRecoverableError).error.statusCode
    ), (
      0,
      403
    ));
    ...
    await notifier.load(search: '');
    expect(notifier.state.items.map((item) => item.nombre), ['A', 'B', 'C']);
    await notifier.select('missing');
    final failure =
        notifier.state.detailState as OperationRecoverableError<CuentaDetalle>;
    expect(
        (failure.error.message, failure.error.statusCode, failure.error.path),
        ('La cuenta no existe', 404, '/cuentas/missing'));
    expect(notifier.state.listState, isA<OperationPartial<List<Cuenta>>>());
    expect((
      RouteAccessPolicy.canAccess(RoutePaths.cuentas,
          role: 'ADMIN', permissions: const {'cuentas:leer'}),
      RouteAccessPolicy.canAccess(RoutePaths.cuentas,
          role: 'VENDEDORA', permissions: const {'cuentas:leer'})
    ), (
      true,
      false
    ));
  });
  testWidgets('read harness keeps newest scoped search and detail response',
      (tester) async {
    final initial = Completer<Object?>(),
        olderSearch = Completer<Object?>(),
        newestSearch = Completer<Object?>(),
        oldDetail = Completer<Object?>(),
    ...
    final repo = CuentasRepository(ApiClient.instance, request: (path, query) {
      ...
      return switch (query['search']) {
        'older' => olderSearch.future,
        'newest' => newestSearch.future,
        _ => initial.future
      };
    });
    final notifier = CuentasNotifier(repo, authorized: true, sedeId: 's1');
    ...
    await tester.pumpWidget(ProviderScope(
        overrides: [cuentasProvider.overrideWith((_) => notifier)],
        child: const MaterialApp(home: CuentasScreen())));
    expect(find.text('Cargando cuentas...'), findsOneWidget);
    initial.complete(
        [account('old', 'Old account'), account('new', 'New account')]);
    await loading;
    await tester.pump();
    await tester.tap(find.text('Old account'));
    await tester.pump();
    await tester.tap(find.text('New account'));
    await tester.pump();
    newDetail.complete(detail('new', 'New account', 22));
    await tester.pump();
    expect(find.text('S/ 22.00'), findsWidgets);
    oldDetail.complete(detail('old', 'Old account', 99));
    await tester.pump();
    expect(find.text('S/ 99.00'), findsNothing);
    ...
    await tester.testTextInput.receiveAction(TextInputAction.search);
    newestSearch.complete([account('scope', 'Scoped result')]);
    await tester.pump();
    olderSearch.complete([account('stale', 'Foreign stale result')]);
    await tester.pump();
    expect((
      find.text('Scoped result').evaluate().length,
      find.text('Foreign stale result').evaluate().length
    ), (
      1,
      0
    ));
  });
  test('collection repository sends exact DTO and maps authoritative response',
      () async {
    late String path;
    ...
    final repo = CuentasRepository(ApiClient.instance,
        ...
        post: (value, payload) async {
          path = value;
          body = payload;
          return paidDetail('c1', 6);
        });
    final result = await repo.collect('c1',
        monto: 4,
        medioPago: 'TRANSFERENCIA',
        idempotencyKey: '11111111-1111-4111-8111-111111111111',
        comprobanteAnalisisId: '22222222-2222-4222-8222-222222222222',
        sedeId: 's1');
    ...
    expect((
      result.saldo,
      result.movimientos.single.tipo,
      result.movimientos.single.raw['medioPago']
    ), (
      6,
      'ABONO',
      'EFECTIVO'
  ...
  test(
    ...
    final deniedRepo = CuentasRepository(ApiClient.instance,
        ...
        post: (_, body) async {
          posts++;
          return paidDetail('c1', 0);
        });
    final denied = CuentasNotifier(deniedRepo,
        authorized: true,
        canCollect: false,
        sedeId: 's1',
        keyFactory: () => 'denied-key');
    await denied.select('c1');
    await denied.collect(monto: 10, medioPago: 'EFECTIVO');
    expect((
      posts,
      denied.state.collectionError?.statusCode,
      (denied.state.detailState as OperationContent<CuentaDetalle>).data.saldo
    ), (
      0,
      403,
      10
    ...
    for (final status in [400, 403, 404, 409]) {
      final repo = CuentasRepository(ApiClient.instance,
          request: (_, __) async => detail('c1', 'Ana'),
          post: (_, __) async => throw AppException(
              message: 'error-$status',
              statusCode: status,
              path: '/cuentas/c1/pagos',
              code: 'E$status'));
      final notifier = CuentasNotifier(repo,
          authorized: true,
          canCollect: true,
          sedeId: 's1',
          keyFactory: () => 'key-$status');
      await notifier.select('c1');
      await notifier.collect(monto: 4, medioPago: 'EFECTIVO');
    ...
    final repo = CuentasRepository(ApiClient.instance,
        ...
        post: (_, body) {
          posts++;
          keys.add(body['idempotencyKey'] as String);
          if (failOnce) {
            failOnce = false;
    ...
    pending.complete(paidDetail('c1', 6));
    await Future.wait([first, duplicate]);
    expect((
      posts,
      notifier.state.collectionSucceeded,
      (notifier.state.detailState as OperationContent<CuentaDetalle>).data.saldo
    ), (
      2,
      true,
      6
  ...
  testWidgets(
      'collection harness applies partial and full payment once per action',
      (tester) async {
    ...
    final repo = CuentasRepository(ApiClient.instance,
        request: (path, __) async =>
            path == '/cuentas' ? [account('c1', 'Ana')] : detail('c1', 'Ana'),
        post: (_, body) {
          payloads.add(body);
    ...
    final notifier = CuentasNotifier(repo,
        ...
        sedeId: 's1',
        keyFactory: () =>
            '00000000-0000-4000-8000-${(++key).toString().padLeft(12, '0')}');
    await notifier.load();
    await notifier.select('c1');
    await tester.pumpWidget(ProviderScope(
        overrides: [cuentasProvider.overrideWith((_) => notifier)],
        child: const MaterialApp(home: CuentasScreen())));
    await tester.tap(find.byKey(const Key('collection-open')));
    ...
    responses.single.complete(paidDetail('c1', 6));
    await tester.pumpAndSettle();
    expect(
        (notifier.state.detailState as OperationContent<CuentaDetalle>)
            .data
            .saldo,
        6);
    expect(notifier.state.items.single.saldo, 6);
    await tester.drag(find.byType(ListView), const Offset(0, -300));
    await tester.pump();
    expect(find.text('S/ 6.00'), findsWidgets);
    await tester.tap(find.byKey(const Key('collection-open')));
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('collection-submit')));
    await tester.pump();
    expect((payloads.length, payloads.last['monto']), (2, 6));
    responses.last.complete(paidDetail('c1', 0));
    await tester.pumpAndSettle();
    expect(find.text('S/ 0.00'), findsOneWidget);
    expect(notifier.state.items, isEmpty);
    expect(payloads.map((item) => item.keys.toSet()).toList(),
        everyElement({'monto', 'medioPago', 'idempotencyKey', 'sedeId'}));
    expect(payloads[0]['idempotencyKey'], isNot(payloads[1]['idempotencyKey']));
  });
  testWidgets(
      'screen mirrors web without direct account creation or local pagination',
      (tester) async {
    final repo = CuentasRepository(ApiClient.instance,
        request: (path, _) async => path == '/cuentas'
            ? [account('c1', 'Ana'), account('c2', 'Beto')]
            : detail('c1', 'Ana'));
    final notifier =
        CuentasNotifier(repo, authorized: true, canCreate: true, sedeId: 's1');
    await notifier.load();
    await tester.pumpWidget(ProviderScope(
        overrides: [cuentasProvider.overrideWith((_) => notifier)],
        child: const MaterialApp(home: CuentasScreen())));
  ...
  testWidgets(
      'transfer payment analyzes a real receipt and uses detected amount',
      (tester) async {
    final payloads = <Map<String, dynamic>>[];
    final repo = CuentasRepository(ApiClient.instance,
    ...
    await notifier.load();
    await notifier.select('c1');
    final ventas = _ReceiptRepository();
    await tester.pumpWidget(ProviderScope(overrides: [
      cuentasProvider.overrideWith((_) => notifier),
      ventasRepositoryProvider.overrideWithValue(ventas),
      voucherImagePickerProvider
          .overrideWithValue(() async => PickedUploadImage(
              bytes: base64Decode(
    ...
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('collection-voucher-pick')));
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('receipt-analysis-panel')), findsOneWidget);
    expect(find.text('Monto: 6.50'), findsOneWidget);
    await tester.tap(find.byKey(const Key('collection-submit')));
    await tester.pumpAndSettle();
    ...
    expect(find.text('Pago registrado. Saldo: S/ 3.50.'), findsOneWidget);
    expect(ventas.cancelled, isEmpty);
  });
}

class _ReceiptRepository extends VentasRepository {
  final List<String> cancelled = [];
  _ReceiptRepository() : super(ApiClient.instance);
  ...
  Future<ComprobanteAnalisis> analizarComprobante({
    ...
    String? sedeId,
      ...
      const ComprobanteAnalisis(
          ...
          imagenUrl: '',
          thumbnailUrl: '',
          confianza: ComprobanteConfianza(
              documento: 1, entidad: 1, monto: 1, operacion: 1, fecha: 1),
          advertencias: []);
